...
2. **Contracts:** Map requirements to observable contracts (input/condition and expected result). Link each contract to an invariant and test path/name when applicable. For non-executable contracts, record the verification method. Add boundary, failure, compatibility, or integration coverage only when established by requirements or repository evidence.
...
- Claims: classify material facts as `Confirmed`, `Assumption`, or `Open decision`.
- Compatibility: flag changes to public inputs/outputs, error meaning, state transitions, persistence schemas, authentication, or authorization as breaking changes and record the required approval and migration/rollback decisions.

## Contracts

| ID | Behavior | Condition or Input | Expected Result | Invariant | Test path/name | Verification |
| --- | --- | --- | --- | --- | --- | --- |
